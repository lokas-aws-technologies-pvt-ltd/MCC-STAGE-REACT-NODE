...
router.route('/item_add').post(upload.single('file'), function (req, res) {
	...
	if (req.file) {
		var filename = req.file.filename;
		var sql = "INSERT INTO rest_menu_item ( item_name,cat_id,sub_cat_id,description,varient,price, active, item_image, item_kitchen,availability_time,banquet_type) VALUES   ('" + name + "','" + cat_id + "','" + sub_cat_id + "'," + description + "','" + varient + "','" + price + "', '" + active + "','" + filename + "','" + kitchen + "','" + availability_time + "','" + banquet_type + "')";
	} else {
		var sql = "INSERT INTO rest_menu_item ( item_name,cat_id,sub_cat_id,description,varient,price, active,  item_kitchen,availability_time,banquet_type) VALUES   ('" + name + "','" + cat_id + "','" + sub_cat_id + "','" + description + "','" + varient + "','" + price + "', '" + active + "','" + kitchen + "','" + availability_time + "','" + banquet_type + "')";
	}
...
router.route('/item_update').post(upload.single('file'), function (req, res) {
	...
	if (req.file) {
		var filename = req.file.filename;
		var sql = "update rest_menu_item set  item_name='" + name + "',item_image='" + filename + "',item_kitchen='" + kitchen + "', cat_id='" + cat_id + "',sub_cat_id='" + sub_cat_id + "',description='" + description + "',varient='" + varient + "',price='" + price + "', active='" + active + "',availability_time='" + availability_time + "',banquet_type='" + banquet_type + "' where item_id='" + id + "'";
	} else {
		var sql = "update rest_menu_item set  item_name='" + name + "',item_kitchen='" + kitchen + "', cat_id='" + cat_id + "',sub_cat_id='" + sub_cat_id + "',description='" + description + "',varient='" + varient + "',price='" + price + "', active='" + active + "',availability_time='" + availability_time + "',banquet_type='" + banquet_type + "' where item_id='" + id + "'";
	}
...
router.route('/order_add').post(function (req, res) {
	...
	if (req.body.clubPickup === 1 ) {
		...
		if(req.body.clubPickup === 2)
		pickup_time="";
		var sql = "INSERT INTO rest_order ( member_code,is_club_pickup,pickup_time, order_status,order_time,total_items, final_price, price,created_by) VALUES   ('" + member_code + "','" + clubPickup + "', '" + pickup_time + "', '" + order_status + "','" + order_time + "','" + cartTotalQuantity + "','" + cartTotalAmount + "','" + cartTotalAmount + "','" + created_by + "');";
			...
			if (result.insertId) {
				...
				if (lastid) {
					sqlms.connect(config, (err) => {
						if (err) {
							...
							new sqlms.Request().query("INSERT INTO rest_order (order_id, member_code,is_club_pickup,pickup_time, order_status,order_time,total_items, final_price, price) VALUES   ('" + lastid + "','" + member_code + "','" + clubPickup + "', '" + pickup_time + "', '" + order_status + "','" + order_time + "','" + cartTotalQuantity + "','" + cartTotalAmount + "','" + cartTotalAmount + "');").then(function (err, result) {
								//...
							});
						}
					});
				...
				for (cartItem of cartItems) {
					...
					var itemsql = "INSERT INTO rest_in_order (order_id, item_id, member_code,quantity,item_price,price,detalis) VALUES   ('" + lastid + "', '" + cartitemid + "','" + member_code + "','" + cartquantity + "','" + price + "','" + itemTotal + "','" + jsoncart + "');";
					 rest_in_order_mssql_insert(itemsql,lastid,member_code,cartItem)
		...
		var sql = "INSERT INTO rest_order ( member_code,delivery_address,order_status,order_time,total_items, final_price, price,created_by) VALUES   ('" + member_code + "','" + delivery_address + "','" + order_status + "','" + order_time + "','" + cartTotalQuantity + "','" + cartTotalAmount + "','" + cartTotalAmount + "','" + created_by + "');";
			...
			if (result.insertId) {
				...
				sqlms.connect(config, (err) => {
					if (err) {
						...
						new sqlms.Request().query("INSERT INTO rest_order (order_id, member_code,delivery_address,order_status,order_time,total_items, final_price, price) VALUES   ('" + lastid + "','" + member_code + "','" + delivery_address + "','" + order_status + "','" + order_time + "','" + cartTotalQuantity + "','" + cartTotalAmount + "','" + cartTotalAmount + "')").then(function (err, result) {
							//...
						});
					}
				});
...
async function rest_in_order_mssql_insert(itemsql,lastid,member_code,cartItem)
{
	await db.query(itemsql, function (erra, resulta) {
		if (erra) return res.status(401).send({ error: erra.message });
		let inid = resulta.insertId;
		sqlms.connect(config, (err) => {
			if (err) {
				console.log(err);
			} else {
				...
				var cartitemid = cartItem.item_id;
					...
					var itemTotal = cartquantity * price;
					var jsoncart = JSON.stringify(cartItem);
				new sqlms.Request().query("INSERT INTO rest_in_order (in_order_id,order_id, item_id, member_code,quantity,item_price,price,detalis) VALUES   ('" + inid + "','" + lastid + "', '" + cartitemid + "','" + member_code + "','" + cartquantity + "','" + price + "','" + itemTotal + "','" + jsoncart + "')").then(function (err, result) {
					//...
...
router.route('/order_update').get(function (req, res) {
	...
	sqlms.connect(config, (err) => {
		if (err) {
			...
			console.log("{ db_Connected! }");
			new sqlms.Request().query(sql).then(function (err, result) {
				//...
			});
		}
...
router.route('/order_status_update').post(function (req, res) {
	...
	sqlms.connect(config, (err) => {
		if (err) {
			...
			if(orderstatus=='5')
			{
				new sqlms.Request().query("update por_tally_data_processed set order_status_flag='DEL' where por_order_id IN(" + ids + ")").then(function (err, result) {
					//...
				});
			}
		}
	...
	db.query(sql, function (err, result) {
		...
		sqlms.connect(config, (err) => {
			if (err) {
				...
				console.log("{ db_Connected! }");
				new sqlms.Request().query(itemsql).then(function (err, result) {
					//...
				});
			}
