...
import { MENU_PLACEMENT, MENU_BEHAVIOUR, NAV_COLOR, THEME_COLOR } from 'constants.js';
...
import { checkBehaviour, checkPlacement, isDeeplyDiffBehaviourStatus, isDeeplyDiffPlacementStatus } from '../main-menu/helper';
import { settingsChangeColor, settingsChangeNavColor } from '../../../settings/settingsSlice';

const MemberMainMenu = () => {
  ...
  const menuItemsMemo = useMemo(
    () =>
      getMenuItems({
        data: attrMobile && useSidebar ? routesAndMenuItems : routesAndMenuItems.memberMainMenuItems,
        isLogin,
  ...
  useEffect(() => {
    dispatch(settingsChangeColor(THEME_COLOR.LightRed));
    dispatch(settingsChangeNavColor(NAV_COLOR.Default));
    dispatch(menuChangePlacement(MENU_PLACEMENT.Vertical));
  }, []);
  useEffect(() => {
      ...
      return (
        <div className="menu-container flex-grow-1">
          <ul id="menu" className={classNames('menu show')}>
            ...
            <li>
              <a href={`https://library.madrascricketclub.org/panel/login/singleSignOn/${currentUser.membercode}`} target="_blank" rel="noopener noreferrer">
                <CsLineIcons icon="book" size="18" className="cs-icon icon" />
...
export default React.memo(MemberMainMenu);
