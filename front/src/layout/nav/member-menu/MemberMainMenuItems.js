...
const MemberMainMenuItems = React.memo(({ menuItems = [] }) =>
  menuItems.map((item, index) => <MemberMainMenuItem key={`menu.${item.path}.${index}`} id={item.path} item={item} />)
);
MemberMainMenuItems.displayName = 'MemberMainMenuItems';

const MemberMainMenuItem = ({ item, id }) => {
  ...
  if (item.subs) {
    return (
      <li>
        ...
        <ul>
          <MemberMainMenuItems menuItems={item.subs} />
        </ul>
      </li>
    );
  }
...
export default MemberMainMenuItems;
