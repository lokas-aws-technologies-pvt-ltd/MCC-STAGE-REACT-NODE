...
const ButtonsCheckAll = ({ tableInstance, deleteItems, changeStatus }) => {
  ...
  const onClick = () => {
    deleteItems({ ids: selectedFlatRows.map((x) => x.original.rsvp_id) });
  };

  const changeTagSelectedItems = (tag) => {
    changeStatus({ ids: selectedFlatRows.map((x) => x.original.rsvp_id), orderstatus:tag})
  };
  return (
    <div className="">
