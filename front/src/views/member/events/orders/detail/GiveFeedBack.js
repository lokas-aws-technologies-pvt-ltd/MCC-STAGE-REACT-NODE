...
const GiveFeedBack = ({ eventOrders, feedback, selectedEventOrder, isOpenGiveFeedBackModal, setIsOpenGiveFeedBackModal }) => {
  // const el = eventOrders.findIndex((obj) => obj.id === selectedEventOrder);
 // console.log('eventOrders', eventOrders.resulta[0]);
  // const currentOrder = eventOrders[el];
// const currentOrder = eventOrders.result;
  const { isLogin, currentUser } = useSelector((state) => state.auth);
  const emptyItem = {
    order_id: '',
    member_code: '',
    eventOrder_name: '',
    feedback_cat:'eventsorder',
    feedback_message: '', 
    member_name:'',
    member_email:'',
    member_phone:'',
  ...
  useEffect(() => {
    if (selectedEventOrder) {
      // console.log('currentOrder', currentOrder);
      setFieldValue('order_id', selectedEventOrder, false);
      setFieldValue('member_code', currentUser.membercode, false);
      setFieldValue('eventOrder_name', eventOrders.resulta?eventOrders.resulta[0].event_name:'' , false);
  ...
  }, [selectedEventOrder, currentUser, setFieldValue]);
  ...
  return (
    ...
    >
      <Modal.Header>
        <Modal.Title>Feed Back of Event for {eventOrders.resulta?eventOrders.resulta[0].event_name:''} </Modal.Title>
      </Modal.Header>
      <Modal.Body>
