...
const OrdersList = () => {
  const title = 'My Event Bookings';
  const description = 'Event Bookings List Page';
  const { isLogin, currentUser } = useSelector((state) => state.auth);
  const breadcrumbs = [
    { to: '', text: 'Home' },
    { to: '', text: 'Events' },
    { to: '', text: 'Booking List' },
  ];
    ...
    return [
      {
        ...
        Cell: ({ row }) => {
          // console.log(row);
          return (
            <a className="list-item-heading body" href={`/member/events/orders/detail/${row.original.rsvp_id}`}>
  ...
  const tableInstance = useTable(
    {
      ...
      pageCount,
      initialState: { pageIndex: 0, sortBy: [{ id: 'rsvp_id', desc: true }], hiddenColumns: [] },
    },
    useGlobalFilter,
    useSortBy,
    usePagination,
  ...
  const fetchData = React.useCallback(async () => {
    ...
    const response = await axios.get(`${API_URL}events/rsvp_get`, { params: { CurrentUserId, term, sortBy, pageSize, pageIndex } });
