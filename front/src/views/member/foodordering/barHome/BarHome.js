...
import { catImageLivePath } from 'constants.js';
import Layout from '../../../../layout/Layout';
import { addToCart, clearCart, decreaseCart, getTotals, removeFromCart } from '../barCart/barCartSlice';
import { getBarProducts } from '../../../../api/barProducts';
...
const BarHome = () => {
  ...
  const fetchCategoryData = React.useCallback(async () => {
    ...
    const response = await axios.get(`${API_URL}bar/category_get`, { params: { active: 1, sortBy } });
  ...
  const fetchSubCategoryData = React.useCallback(async () => {
    if (currentCat != '') {
      const response = await axios.get(`${API_URL}bar/sub_category_get`, { params: { active: 1, catid: currentCat } });
  ...
  const loadProducts = () => {
    ...
    const active = 1;
    setTimeout(() => {
      getBarProducts(term, currentCat, currentSubCat, pageSize, pageIndex, foodType, foodAvailability, active)
        .then((res) => {
          // ('pageIndex', pageIndex);
          ...
          <Row className="g-0">
            ...
            <Col xs="12" o sm="auto" className="d-flex align-items-end justify-content-end mb-2 mb-sm-0 order-sm-3" style={{ position: 'relative' }}>
              <NavLink
                ...
                to="/member/foodordering/barCart"
            ...
            subCatData.map(function (value, i) {
              ...
              return (
                <Col key={`cat_${value.sub_id}`} className="sh-15 catCol">
                  ...
                  >
                    <Card.Title
                      className={currentSubCat === value.sub_id ? `h-100 hover-border-primary` : `h-100 hover-border-primary cat`}
                      style={{
                        minHeight: 120,
                        padding: '20% 20%',
                        margin: '0 auto',
          ...
          {data.length > 0
            ? // eslint-disable-next-line func-names
              data.map(function (product, i) {
                const existingIndex = barCart.barCartItems.findIndex((item) => item.item_id === product.item_id);
                          ...
                          <Col>
                            {existingIndex < 0 ? (
                              ...
                              <InputGroup className="spinner sw-11" style={{ position: 'relative', display: 'inline-flex', height: '0px', background: '#fff' }}>
                                ...
                                <Form.Control
                                  value={barCart.barCartItems[existingIndex].barCartTotalQuantity}
                                  onInput={(e) => {
                                    onInput(e, barCart.barCartItems[existingIndex].barCartTotalQuantity, product);
                                  }}
                                  placeholder="Count"
