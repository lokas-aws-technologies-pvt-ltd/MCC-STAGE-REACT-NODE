...
import { userLoginadminmember } from '../../api/authapi';
...
const Loginadminmember = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    admincode: Yup.string().required('Admincode is required'),
    pin: Yup.number().positive().integer().required('Pin Number is required'),
  });
  const initialValues = { membercode: '', admincode: '', pin: '' };
  useEffect(() => {
  ...
  const onSubmit = async (e) => {
   ...
   // console.log(e);
    ...
    try {
      ...
      const isAuth = await userLoginadminmember(e);
      ...
      else if(isAuth.success === '2'){
        return dispatch(loginFail('Please Enter Correct Member Code '));
      }
      else {
        return dispatch(loginFail('Please Enter valid Credentials '));
      }
      ...
      <div className="sw-lg-50 px-5">
        ...
        <div>
          <form id="loginForm" className="tooltip-end-bottom" onSubmit={handleSubmit}>
            ...
            <div className="mb-3 filled form-group tooltip-end-top">
              <CsLineIcons icon="user" />
              <Form.Control type="admincode" name="admincode" onChange={handleChange} value={values.admincode} placeholder="admincode" />
            
              {errors.admincode && touched.admincode && <div className="d-block invalid-tooltip">{errors.admincode}</div>}
            </div>
            <div className="mb-3 filled form-group tooltip-end-top">
...
export default Loginadminmember;
