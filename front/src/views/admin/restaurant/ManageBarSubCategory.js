...
import BarSubCategoryAddEdit from './BarSubCategoryAddEdit';
...
const ManageBarSubCategory = () => {
  ...
  const fetchData = React.useCallback(async () => {
    document.body.classList.add('spinner');
    const response = await axios.get(`${API_URL}bar/sub_category_get`, { params: { catid: id, term, sortBy, pageSize, pageIndex } });
  ...
  const addItem = React.useCallback(
    async ({ item }) => {
      document.body.classList.add('spinner');
      const response = await axios.post(`${API_URL}bar/sub_category_add`, {  item });
      setTimeout(() => {
        fetchData();
        document.body.classList.remove('spinner');
      }, 1000);
  ...
  const editItem = React.useCallback(
    async ({ item }) => {
      document.body.classList.add('spinner');
      const response = await axios.post(`${API_URL}bar/sub_category_update`, { item });
      setTimeout(() => {
       fetchData();
        document.body.classList.remove('spinner');
      }, 1000);
  ...
  const deleteItems = React.useCallback(
    async ({ ids }) => {
      document.body.classList.add('spinner');
      const response = await axios.post(`${API_URL}bar/sub_category_delete`, { ids });
      setTimeout(() => {
        fetchData();
        document.body.classList.remove('spinner');
      }, 1000);
      ...
      <Layout>
        <Row>
          <Col>
            ...
            <BarSubCategoryAddEdit tableInstance={tableInstance} addItem={addItem} editItem={editItem} />
          </Col>
        </Row>
        <Row />
        <br />
...
export default ManageBarSubCategory;
