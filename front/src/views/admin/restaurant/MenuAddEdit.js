...
const MenuAddEdit = ({ tableInstance, addItem, editItem }) => {
  const { selectedFlatRows, catData,banData, setIsOpenAddEditModal, isOpenAddEditModal } = tableInstance;
  const emptyItem = {
    item_image: '',
    item_id: '',
    item_name: '',
  ...
  const fetchSubCategoryData = React.useCallback(async () => {
    ...
    const response = await axios.get(`${API_URL}restaurant/sub_category_get`, { params: { catid: catId } });
  ...
  const changebanquet = (event) => {
   
    setSelectedItem({ ...selectedItem, banquet_type: event.target.value });
  
};
  useEffect(() => {
    if (selectedFlatRows.length === 1) {
      // get user and set form fields
      const fields = ['item_image', 'item_id', 'item_name', 'cat_id', 'sub_cat_id', 'varient','banquet_type', 'active', 'description', 'price', 'availability_time'];
    ...
    <Modal className="modal-right" show={isOpenAddEditModal} onHide={() => setIsOpenAddEditModal(false)}>
      ...
      <Modal.Body>
        <Form onSubmit={handleSubmit}>
          ...
          <div className="mb-3">
            <Form.Label>Food Sub Category</Form.Label>
          ...
          <div className="mb-3">
            <Form.Label>Banquet Course</Form.Label>
            <Form.Select value={values ? values.banquet_type : selectedItem.banquet_type} name="banquet_type" onChange={(e) => {
                handleChange(e);
                 changebanquet(e);
              }}>
              <option key="0" value="">
                Select Banquet Course
              </option>
              {banData.map((x, y) => (
                <option key={x.banquet_menu_course_id} value={x.banquet_menu_course_id}>
                  {x.banquet_menu_course_name}
                </option>
              ))}
            </Form.Select>
            {errors.banquet_type && touched.banquet_type && (
              <div style={{ position: 'relative' }} className="d-block invalid-tooltip">
                {errors.banquet_type}
              </div>
            )}
