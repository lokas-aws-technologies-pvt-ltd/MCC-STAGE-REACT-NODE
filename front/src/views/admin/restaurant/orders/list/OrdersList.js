...
const OrdersList = () => {
    ...
    return [
      {
        ...
        Cell: ({ row }) => {
          // console.log(row);
          return (
            <a className="list-item-heading body" href={`/admin/restaurant/OrderDetail/${row.original.order_id}`}>
  ...
  const fetchData = React.useCallback(async () => {
    ...
    const response = await axios.get(`${API_URL}restaurant/order_get`, { params: { term, sortBy, pageSize, pageIndex } });
...
setTerm('');
  ...
  const deleteItems = React.useCallback(
    async ({ ids }) => {
      document.body.classList.add('spinner');
      const response = await axios.post(`${API_URL}restaurant/order_delete`, { ids });
      setTimeout(() => {
