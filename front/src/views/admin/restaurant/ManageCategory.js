...
const ManageCategory = () => {
  const title = 'Manage Menu Categories';
  const description = '';
  const breadcrumbs = [
    { to: '', text: 'Home' },
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          ...
          return (
            <a
              className="list-item-heading body"
              href={`/admin/restaurant/ManageSubCategory/${row.original.id}`}
              onClick={(e) => {
                //  e.preventDefault();
              }}
            >
  ...
  const fetchData = React.useCallback(async () => {
    ...
    const response = await axios.get(`${API_URL}restaurant/category_get`, { params: { term, sortBy, pageSize, pageIndex } });
  ...
  const addItem = React.useCallback(
    async ({ item }) => {
      ...
      const response = await axios.post(`${API_URL}restaurant/category_add`, formData, uploadConfig);
      setTimeout(() => {
  ...
  const editItem = React.useCallback(
    async ({ item }) => {
      ...
      const response = await axios.post(`${API_URL}restaurant/category_update`, formDataOne, uploadConfig);
      setTimeout(() => {
  ...
  const deleteItems = React.useCallback(
    async ({ ids }) => {
      document.body.classList.add('spinner');
      const response = await axios.post(`${API_URL}restaurant/category_delete`, { ids });
      setTimeout(() => {
