...
import BarMenuAddEdit from './BarMenuAddEdit';
...
const BarManageMenu = () => {
  ...
  const fetchSubCategoryData = React.useCallback(async () => {
    document.body.classList.add('spinner');
    const response = await axios.get(`${API_URL}bar/sub_category_get`);
  ...
  const fetchData = React.useCallback(async () => {
    document.body.classList.add('spinner');
    // console.log(`${API_URL}/bar/item_get`);
    // const response = await axios.get(`${SERVICE_URL}/datatable`, { params: { term, sortBy, pageSize, pageIndex } });
    const response = await axios.get(`${API_URL}bar/item_get`, { params: { term, sortBy, pageSize, pageIndex } });
  ...
  const addItem = React.useCallback(
    async ({ item }) => {
      ...
      const response = await axios.post(`${API_URL}bar/item_add`, formData, uploadConfig);
      setTimeout(() => {
  ...
  const editItem = React.useCallback(
    async ({ item }) => {
      ...
      const response = await axios.post(`${API_URL}bar/item_update`, formDataOne, uploadConfig);
      setTimeout(() => {
  ...
  const deleteItems = React.useCallback(
    async ({ ids }) => {
      // console.log('ids', ids);
      document.body.classList.add('spinner');
      const response = await axios.post(`${API_URL}bar/item_delete`, { ids });
      setTimeout(() => {
      ...
      <Layout>
        <Row>
          <Col>
            ...
            <BarMenuAddEdit tableInstance={tableInstance} addItem={addItem} editItem={editItem} />
          </Col>
        </Row>
        <Row />
        <br />
