    ...
    return [
	  ...
	  let comp;
          if (row.original.feedback_category == 'eventsorder') {
             comp = <a className="list-item-heading body" href={`/admin/events/orderDetail/${row.original.ref_id}`}> {row.original.ref_id} </a>
            
          } else if(row.original.feedback_category == 'f&b') {
		 comp = <a className="list-item-heading body" href={`/admin/restaurant/OrderDetail/${row.original.ref_id}`}> {row.original.ref_id} </a>
...
cell.value
  ...
  const tableInstance = useTable(
    {
      ...
      pageCount,
      initialState: { pageIndex: 0, sortBy: [{ id: 'feedback_id', desc: false }], hiddenColumns: [] },
    },
    useGlobalFilter,
    useSortBy,
    usePagination,
  ...
  const fetchData = React.useCallback(async () => {
    ...
    const response = await axios.get(`${API_URL}feedbacks/feedback_get`, { params: { term, sortBy, pageSize, pageIndex } });
