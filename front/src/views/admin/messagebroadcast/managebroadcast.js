...
import Broadcastaddedit from './broadcastaddedit';
...
const managebroadcast = () => {
  const title = 'Manage Broadcast';
  const description = 'Manage Broadcast';
  ...
  const columns = React.useMemo(() => {
    return [
      ...
      //   },
      // },
      { Header: 'imagepath', accessor: 'imagepath',  sortable: true,
      headerClassName: 'text-muted text-small text-uppercase w-10',
      Cell: ({ cell }) => {
        return (
          <img
            src={cell.value ? `${broadcastPath}${cell.value}` : '/img/product/small/product-1.webp'}
            alt="image" 
            className="card-img card-img-horizontal sw-11 h-100"
          />
        );
      }, },
      { Header: 'filepath', accessor: 'filepath', Cell: ({ cell }) => {
        return (<a target="_blank" rel="noreferrer" href={`${broadcastPath}${cell.value}`}> {cell.value} </a> );
      }, sortable: true, headerClassName: 'text-muted text-small text-uppercase w-10' },
      { Header: 'message', accessor: 'message', sortable: true, headerClassName: 'text-muted text-small text-uppercase w-30' },
      { Header: 'group', accessor: 'groupid', sortable: true, headerClassName: 'text-muted text-small text-uppercase w-10' },
      { Header: 'medium', accessor: 'medium', sortable: true, headerClassName: 'text-muted text-small text-uppercase w-10' },
      { Header: 'date', accessor: 'broadcast_date', sortable: true, headerClassName: 'text-muted text-small text-uppercase w-20' },
      {
        Header: '',
        id: 'action',
  ...
  const tableInstance = useTable(
    {
      ...
      pageCount,
      initialState: { pageIndex: 0, sortBy: [{ id: 'id', desc: false }], hiddenColumns: [] },
    },
    useGlobalFilter,
    useSortBy,
    usePagination,
  ...
  const fetchData = React.useCallback(async () => {
    ...
    const response = await axios.get(`${API_URL}broadcast/get_broadcast`, { params: { term, sortBy, pageSize, pageIndex } });
  ...
  const addItem = React.useCallback(
    async ({ item }) => {
      ...
      const response = await axios.post(`${API_URL}broadcast/create_broadcast`, formData, uploadConfig);
      setTimeout(() => {
toast.success('Broadcast Message Added Successfully', {
          position: 'top-right',
        });
setTerm('');
        fetchData();
  ...
  const editItem = React.useCallback(
    async ({ item }) => {
      ...
      const response = await axios.post(`${API_URL}broadcast/update_broadcast`,  formDataOne, uploadConfig);
      setTimeout(() => {
...
searchItem('');
  ...
  const deleteItems = React.useCallback(
    async ({ ids }) => {
      document.body.classList.add('spinner');
      const response = await axios.post(`${API_URL}broadcast/delete_broadcast`, { ids });
      setTimeout(() => {
      ...
      <Layout>
        <Row>
          <Col>
            ...
            <Broadcastaddedit tableInstance={tableInstance} addItem={addItem} editItem={editItem} />
          </Col>
        </Row>
        <Row />
        <br />
