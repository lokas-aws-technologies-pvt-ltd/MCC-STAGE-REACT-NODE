...
import ImageUploadAddEdit from './ImageUploadAddEdit';
...
const ManageAffiliateClub = () => {
  const title = 'Manage Image Upload';
  const description = 'Manage Image Upload';
  ...
  const columns = React.useMemo(() => {
    return [
      ...
      { Header: 'Category', accessor: 'categoryname', sortable: true, headerClassName: 'text-muted text-small text-uppercase w-10'},
      ...
      Cell: ({ cell }) => {
        return (
          <img
            src={cell.value ? `${imageupload}${cell.value}` : '/img/product/small/product-1.webp'}
            alt="image" 
  ...
  const tableInstance = useTable(
    {
      ...
      pageCount,
      initialState: { pageIndex: 0, sortBy: [{ id: 'A.id', desc: false }], hiddenColumns: ['id'] },
    },
    useGlobalFilter,
    useSortBy,
    usePagination,
  ...
  const fetchData = React.useCallback(async () => {
    ...
    const response = await axios.get(`${API_URL}general/get_image_upload`, { params: { term, sortBy, pageSize, pageIndex } });
  ...
  const addItem = React.useCallback(
    async ({ item }) => {
      ...
      const response = await axios.post(`${API_URL}general/image_upload`,  formData, uploadConfig );
      setTimeout(() => {
...
setTerm('');
  ...
  const editItem = React.useCallback(
    async ({ item }) => {
      ...
      const response = await axios.post(`${API_URL}general/image_upload_update`, formDataOne, uploadConfig);
      setTimeout(() => {
...
searchItem('');
  ...
  const deleteItems = React.useCallback(
    async ({ ids }) => {
      document.body.classList.add('spinner');
      const response = await axios.post(`${API_URL}general/image_upload_delete`, { ids });
      setTimeout(() => {
      ...
      <Layout>
        <Row>
          <Col>
            ...
            <ImageUploadAddEdit tableInstance={tableInstance} addItem={addItem} editItem={editItem} />
          </Col>
        </Row>
        <Row />
        <br />
