  ...
  return (
    <>
      ...
      <Layout>
        ...
        <Tab.Container defaultActiveKey="chamberinfo" onSelect={handleSelect}>
          <Nav variant="tabs" className="nav-tabs-title card-header-tabs" activeKey="chamberinfo" as={ResponsiveNav}>
            <Nav.Item>
              <Nav.Link eventKey="chamberinfo">Chamber Request Info </Nav.Link>
            </Nav.Item>
            <Nav.Item>
              <Nav.Link eventKey="transactioninfo">Transactions Info</Nav.Link>
            </Nav.Item>
            <Nav.Item>
              <Nav.Link eventKey="lastfileinfo">Last Uploaded File Info</Nav.Link>
            </Nav.Item>
          </Nav>
          <Card body className="mb-5">
            <Tab.Content>
              <Tab.Pane eventKey="chamberinfo">
                <Card.Title>Chamber Request Info For {currentMonth} 2022</Card.Title>
                <Table bordered>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    { /* dashboardInfo.chambReq && dashboardInfo.chambReq.length>0 ? (
                      dashboardInfo.chambReq.map((item) => {
                        return (
                          <tr key={item.order_status}>
                ...
                <Card.Title>Chamber Transaction Info For {currentMonth} 2022</Card.Title>
                <Table bordered>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    { /* dashboardInfo.chambOrders && dashboardInfo.chambOrders.length > 0 ? (
                      dashboardInfo.chambOrders.map((item) => {
                        return (
                          <tr key={item.order_status}>
                            <td style={{ textTransform: 'capitalize' }}>{item.order_status}</td>
                            <td>{item.cnt}</td>
                            <td>&#x20B9; {item.amt}</td>
                          </tr>
                        );
			...
			<tr>
              ...
              <Tab.Pane eventKey="transactioninfo">
                <Card.Title>Transaction Info For {currentMonth} 2022</Card.Title>
                <Table bordered>
                  <thead>
                    <tr>
                      <th scope="col">Transaction Details</th>
                  ...
                  <tbody>
                    {arr.length > 0 ? (
                      <>
                        <tr key="0">
                          <td style={{ textTransform: 'capitalize' }}>Successful</td>
                          <td>{arr[1].value}</td>
                          <td>&#x20B9; {arr[0].value}</td>
                        </tr>
                        <tr key="1">
                          <td style={{ textTransform: 'capitalize' }}>Approved</td>
                          <td>{arr[3].value}</td>
                          <td>&#x20B9; {arr[2].value}</td>
                        </tr>

                        <tr key="2">
                          <td style={{ textTransform: 'capitalize' }}>Pending</td>
                          <td>{arr[7].value}</td>
                          <td>&#x20B9; {arr[6].value}</td>
                        </tr>
                        <tr key="3">
                          <td style={{ textTransform: 'capitalize' }}>Failed</td>
                          <td>{arr[5].value}</td>
                          <td>&#x20B9;{arr[4].value}</td>
                        </tr>
                        <tr key="4">
                          <td style={{ textTransform: 'capitalize' }}>Closed</td>
                          <td>{arr[9].value}</td>
                          <td>&#x20B9; {arr[8].value}</td>
                        </tr>
                      </>
              ...
              <Tab.Pane eventKey="lastfileinfo">
                <Card.Title>
                  {dashboardInfo.crmonth && dashboardInfo.crmonth == 'crmonth' ? `Last Uploaded File Info For ${dashboardInfo.billdate}` : `Last Uploaded File`}
                </Card.Title>
                <Table className="table-borderless">
                  <tbody>
                    {dashboardInfo.opbs ? (
                      <>
                        <tr>
                          <td>Billing Month: {dashboardInfo.billdate}</td>
                          <td>Bill uploaded by: {dashboardInfo.uplby}</td>
                        </tr>
                        <tr>
                          <td>No.of.Members: {dashboardInfo.count}</td>
                          <td>Bill uploded on: {dashboardInfo.uploadedon}</td>
                        </tr>
                        <tr>
                          <td>Total Opening Balance: </td>
                          <td>&#x20B9; {dashboardInfo.opbs}</td>
                        </tr>
                        <tr>
                          <td>Total Bill Amount: </td>
                          <td>&#x20B9; {dashboardInfo.billamt}</td>
                        </tr>
                        <tr>
                          <td>Total Received Amount: </td>
                          <td>&#x20B9; {dashboardInfo.receivedamount}</td>
                        </tr>
                        <tr>
                          <td>Total Closing Balance: </td>
                          <td>&#x20B9; {dashboardInfo.clsamt}</td>
                        </tr>
                      </>
                    ) : (
                      <tr>
                        <td colSpan="2" className="c-text">
                          <b>No Data</b>
                        </td>
                      </tr>
                    )}
